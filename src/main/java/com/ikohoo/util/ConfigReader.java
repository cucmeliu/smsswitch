...
public class ConfigReader {
	...
	private static Config getProp() {
		...
		try {
			...
			config.setSendId(prop.getProperty("send-id", "id"));
			config.setSendPhone(prop.getProperty("send-phone", "phone"));
			config.setSendContent(prop.getProperty("send-content", "content"));
			config.setSendIntime(prop.getProperty("send-intime", "intime"));
			config.setSendState(prop.getProperty("send-state", "state"));
			config.setSendSendtime(prop.getProperty("send-sendtime", "sendtime"));
			config.setSendStatcode(prop.getProperty("send-statcode", "statcode"));
			
			config.setTableRecv(prop.getProperty("table-recv", "receivesms_sf"));
			config.setRecvId(prop.getProperty("recv-id", "id"));
			config.setRecvPhone(prop.getProperty("recv-phone", "phone"));
			config.setRecvContent(prop.getProperty("recv-content", "content"));
			config.setRecvSendtime(prop.getProperty("recv-sendtime", "sendtime"));
			config.setRecvSystime(prop.getProperty("recv-systime", "systime"));
			
			config.setTableRept(prop.getProperty("table-rept", "stat_sf"));
			config.setReptId(prop.getProperty("rept-id", "id"));
			config.setReptPhone(prop.getProperty("rept-phone", "phone"));
			config.setReptStatCode(prop.getProperty("rept-statcode", "statcode"));
			config.setReptStatMsg(prop.getProperty("rept-statmsg", "statmsg"));
			config.setReptSmsId(prop.getProperty("rept-smsid", "smsid"));
			config.setReptSendTime(prop.getProperty("rept-sendtime", "sendtime"));
			
			config.setTableSent(prop.getProperty("table-sent", "send_sf_done"));
			
			config.setSendSeqVal(prop.getProperty("send-seq-val", "send_sf_seq.nextval"));
			config.setSendDoneSeqVal(prop.getProperty("send-done-seq-val", "send_sf_done_seq.nextval"));
			config.setReceivesmsSeqVal(prop.getProperty("receivesms-seq-val", "receivesms_sf_seq.nextval"));
			config.setStatSeqVal(prop.getProperty("stat-seq-val", "stat_sf_seq.nextval"));
