...
public class SMSSendServiceImpl implements SMSSendService {
	...
	private int deleteSent1(List<SMSSendBean> list) {
		try {
			// dao.setTable(config.getTableSend());
			return dao.delSentRec1(list);
		} catch (SQLException e) {
			e.printStackTrace();
			logger.error(e);
		}
	...
	public void setConfig(Config config) {
		...
		dao.setSendid(config.getSendId());
		dao.setSendphone(config.getSendPhone());
		dao.setSendcontent(config.getSendContent());
		dao.setSendintime(config.getSendIntime());
		dao.setSendstate(config.getSendState());
		dao.setSendsendtime(config.getSendSendtime());
		dao.setSendstatcode(config.getSendStatcode());
		dao.setSendSeq(config.getSendSeqVal());
		dao.setSendDoneSeq(config.getSendDoneSeqVal());
	}
	...
	public int sendYunXin(List<SMSSendBean> list) {
		...
		if ((null != notFitList) && (notFitList.size() > 0)) {
			//修改个性化打包，大于500时打多个包发送
			while (notFitList.size() > 0) {
				int max=config.getPackMax();
				if(notFitList.size()<config.getPackMax()){
					max=notFitList.size();
				}
			List<SMSSendBean> subList = notFitList.subList(0, max);
			SMSSendParams ssp = new SMSSendParams();

			ssp.setChannel(config.getChannal());
			ssp.setMsg(getOneYunXinPackStr(subList));
			...
			list.addAll(subList);
			notFitList.subList(0, max).clear();
		}
	}
		list.addAll(rstList);
...
public static void main(String[] args) {
	...
	while ( list.size() >0) {
		 ...
		 for (Integer integer : list) {
			System.err.println( "aaaa"+integer);
		}
	}
