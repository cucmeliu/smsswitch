...
public class SMSRecvDaoImplSqlServer implements SMSRecvDao {
	// 接收表名
	...
	public int[] insert(List<SMSRecvBean> list) throws SQLException {
		StringBuilder sqlBuilder = new StringBuilder("INSERT INTO ").append(table).append("(").append(recvphone)
				.append(Constants.SEPARATOR_COMMA).append(recvcontent).append(Constants.SEPARATOR_COMMA)
				.append(recvsendtime).append(Constants.SEPARATOR_COMMA).append(recvsystime).append(") ")
				.append("VALUES (?, ?, ?, ?)");
