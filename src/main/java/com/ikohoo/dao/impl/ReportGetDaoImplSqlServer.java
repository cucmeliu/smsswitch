...
public class ReportGetDaoImplSqlServer implements ReportGetDao {
	...
	public int[] insert(List<ReportGetBean> list) throws SQLException {
		StringBuilder sqlBuilder = new StringBuilder("INSERT INTO ").append(table).append("(").append(reptphone)
				.append(Constants.SEPARATOR_COMMA).append(reptstatcode).append(Constants.SEPARATOR_COMMA)
				.append(reptstatmsg).append(Constants.SEPARATOR_COMMA).append(reptsmsid)
				.append(Constants.SEPARATOR_COMMA).append(reptsendtime).append(") ").append("VALUES (?, ?, ?, ?, ?)");
